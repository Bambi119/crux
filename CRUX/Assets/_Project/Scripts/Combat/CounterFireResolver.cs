...
namespace Crux.Combat
{
    ...
    /// docs/06_combat_system.md 기준:
    /// 1. 방어측 생존
    /// 2. 주포 무손상
    /// 3. 전방 호 ±60° LOS
    /// 4. AP 충분
    /// 5. 사수 무손상 (현재 phase 1: 항상 true)
    /// 6. 반격 면역 아님
    /// 7. 연쇄 반격 차단
    /// 8. Task #17: 플레이어 반격 확정 (플레이어만)
    /// </summary>
    public static class CounterFireResolver
    {
        /// <summary>반격 7조건 검사만. 실제 사격 해결은 FireExecutor가 호출.</summary>
        ...
        public static CounterCheckResult CheckWithReason(GridTankUnit defender, GridTankUnit attacker, GridManager grid)
        {
            ...
            {
                return new CounterCheckResult
                {
                    canCounter = false,
                    reason = CounterFailReason.DefenderDead
                };
            }

            // 2. 주포 무손상 — Barrel/MainGun 모듈 체크
            // ModuleType enum 가져오기
            ...
            if (barrelModule == null || barrelModule.state != ModuleState.Normal)
            {
                ...
                {
                    canCounter = false,
                    reason = CounterFailReason.MainGunDamaged
                };
            }

            // 3. 전방 호 ±60° LOS — 방향 + LOS
            if (!HasLOSInArc(defender, attacker, grid))
            {
                return new CounterCheckResult
                {
                    canCounter = false,
                    reason = CounterFailReason.OutOfArc
                };
            }

            // 4. AP 충분 — GetFireCost() 호출 (modifiers 포함)
            ...
            if (defender.CurrentAP < fireAPCost)
            {
                ...
                {
                    canCounter = false,
                    reason = CounterFailReason.InsufficientAP
                };
            }

            // 5. 사수 무손상 — Phase 1: 항상 true. TODO Phase 2에서 Crew 체크 추가
            // 현재는 skip (TODO: Crew 시스템 통합 시 수정)
            ...
            if (defender.IsCounterImmune)
            {
                ...
                {
                    canCounter = false,
                    reason = CounterFailReason.IsOverwatch
                };
            }

            // 7. 연쇄 반격 차단 — 이번 교환에서 이미 반격했으면 false
            if (defender.HasCounteredThisExchange)
            {
                return new CounterCheckResult
                {
                    canCounter = false,
                    reason = CounterFailReason.ChainBlocked
                };
            }

            // 8. Task #17: 플레이어 반격 확정 — 플레이어 유닛이면 확정 여부 체크
            if (!defender.CounterConfirmed)
            {
                return new CounterCheckResult
                {
                    canCounter = false,
                    reason = CounterFailReason.PlayerDeclined
                };
            }
        ...
        private static bool HasLOSInArc(GridTankUnit defender, GridTankUnit attacker, GridManager grid)
        {
            ...
            {
                return false;  // LOS 없으면 OutOfArc로 반환됨 (위에서 이미 처리)
            }

            // 차체 각도(facing) 기준 ±60° 범위 내인지 확인
            Vector2Int defPos = defender.GridPosition;
            Vector2Int attPos = attacker.GridPosition;

            // 월드 좌표로 변환
            Vector3 defWorldPos = grid.GridToWorld(defPos);
            Vector3 attWorldPos = grid.GridToWorld(attPos);

            // 벡터 계산: 방어측 → 공격측
            Vector2 dirToAttacker = (attWorldPos - defWorldPos).normalized;

            // 방어측 차체 각도를 벡터로 변환 (HullAngle은 compass 기준)
            // compass 0° = 북(↑), 90° = 동(→), 180° = 남(↓), 270° = 서(←)
            float defenderCompassAngle = defender.HullAngle;
            Vector2 defenderFacing = CompassToVector(defenderCompassAngle);
        ...
        {
            // compass: 0° = 북(0, 1), 90° = 동(1, 0), 180° = 남(0, -1), 270° = 서(-1, 0)
            float rad = compassAngle * Mathf.Deg2Rad;
            // sin cos 순서: sin은 Y, cos는 X (standard math)
            // 그러나 Unity는 Y가 아래→위이고, compass 0이 위이므로:
            // 0° → (0, 1), 90° → (1, 0)
...
#if UNITY_EDITOR
        public static void LogDetailedResult(GridTankUnit defender, GridTankUnit attacker, CounterCheckResult result, GridManager grid)
        {
            ...
            float defAngle = defender.HullAngle;
            Vector2Int defPos = defender.GridPosition;
            Vector2Int attPos = attacker.GridPosition;
            Vector3 defWorldPos = grid.GridToWorld(defPos);
            Vector3 attWorldPos = grid.GridToWorld(attPos);
            Vector2 dirToAttacker = (attWorldPos - defWorldPos).normalized;
            Vector2 defFacing = CompassToVector(defAngle);
            float angleToAttacker = Vector2.Angle(defFacing, dirToAttacker);

            Debug.Log($"  3. Arc & LOS: LOS={hasLOS}, Facing={defAngle}°, AngleToAtt={angleToAttacker:F1}°, InArc={angleToAttacker <= 60f}");
            ...
            Debug.Log($"  8. Task #17 CounterConfirmed: {defender.CounterConfirmed}");
    ...
    public enum CounterFailReason
    {
        ...
        AttackerMoving,      // 공격측이 이동 중 (현재 미사용 — 반격자가 이동 중인 경우만 체크)
        PlayerDeclined       // Task #17: 플레이어가 반격 프롬프트에서 거부
    }
}
