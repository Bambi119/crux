...
namespace Crux.UI
{
    ...
    public class UnitInfoCard : MonoBehaviour
    {
        ...
        public void ShowFirePreview(float hitChance, float expectedDamage)
        {
            ...
            // main API: StartFlash(currentHP, maxHP, predictedDamage) — DamageOverlay child Image 사용
            if (flashAnimator != null && boundUnit != null)
            {
                float maxHP = boundUnit.Data != null ? boundUnit.Data.maxHP : 100f;
                flashAnimator.StartFlash(boundUnit.CurrentHP, maxHP, expectedDamage);
            }
        }
