...
### 8.2 모델 지정 (전역 §8과 일치, CRUX 적용)

**역할별 고정 모델**
- **메인(부모 세션) = Opus 4.7** — 검토·검증·계획·통합·에이전트 지시/수정·코드 리뷰 전담. 직접 구현 최소화
- **시타 / 시그마 / 픽셀 / 모나미 = Sonnet 기본** — 구현·탐색·검증 실행 전량

**강제 규칙**
- `Agent` tool 호출 시 **`model: "sonnet"` 파라미터 필수 지정**. 시타·시그마·픽셀·모나미 4종 모두 예외 없음
- 파라미터 누락 = 부모(Opus) 상속 → 비용 폭증. 자동 다운그레이드 안 됨
- 서브에이전트 정의 파일(`.claude/agents/*.md`)의 `model:` frontmatter는 호출 파라미터보다 우선순위 낮음 — **호출 시점 파라미터가 신뢰원**
- 메인이 직접 구현 코드를 작성해야 하는 상황은 **"서브에이전트 결과 통합·미세 수정·최종 검토"에 한정**. 새 파일 신규 작성·대규모 로직 추가·씬 구성은 시그마/픽셀 위임

**예외 (Opus 서브에이전트 허용)**
- 핵심 아키텍처 설계 판단 (BattleController 분할처럼 구조 결정이 프로젝트 수명에 영향)
- 재현 어려운 디버깅 (다중 가설 추론 필요, 예: 반응 사격 타이밍 버그)
- **사용자에게 사전 고지 후** 명시적으로 `model: "opus"` 지정
